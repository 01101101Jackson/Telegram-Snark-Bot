...
bot.onText(/\/start/, (msg) => {
    ...
    // Get or create user
    let user = database_1.db.getUser(chatId);
    if (!user) {
    ...
    if (!user.userGender || !user.targetGender) {
        ...
        bot.sendMessage(chatId, '👑 **WHO ARE YOU?**', { parse_mode: 'Markdown', reply_markup: configKeyboard });
    }
});
...
bot.onText(/\/language/, (msg) => {
    ...
    let user = database_1.db.getUser(chatId);
    if (!user) {
        user = database_1.db.createUser(chatId, msg.from?.username);
    }
    const languageMessage = (0, languages_1.getMessage)(user.language, 'chooseLanguage');
    const keyboard = {
        inline_keyboard: Object.entries(languages_1.languages).map(([code, lang]) => [
...
bot.onText(/\/subscribe/, (msg) => {
    ...
    const subscribeMessage = (0, languages_1.getMessage)(user.language, 'subscribeInfo');
    const keyboard = {
        inline_keyboard: [
...
bot.onText(/\/admin_vip (\d+)/, (msg, match) => {
    ...
    const success = database_1.db.grantVip(targetUserId);
    if (success) {
...
bot.onText(/\/admin_revoke (\d+)/, (msg, match) => {
    ...
    const targetUserId = parseInt(match[1]);
    const success = database_1.db.revokeVip(targetUserId);
    if (success) {
...
bot.onText(/\/admin_vips/, (msg) => {
    ...
    const vips = database_1.db.getVipUsers();
    if (vips.length === 0) {
...
bot.onText(/\/admin_users/, (msg) => {
    ...
    const allUsers = database_1.db.getAllUsers();
    const stats = {
...
bot.onText(/\/admin_activate (\d+)/, (msg, match) => {
    ...
    const targetUserId = parseInt(match[1]);
    database_1.db.activateSubscription(targetUserId);
...
bot.onText(/\/reset/, (msg) => {
    ...
    database_1.db.clearChatHistory(chatId);
    database_1.db.updateUser(chatId, {
        userGender: undefined,
        targetGender: undefined
    });
    bot.sendMessage(chatId, `🔄 **RESET COMPLETE**\n\nChat history cleared. Fresh start.\n\nSend a message and I'll ask who you are and who you're texting.`, { parse_mode: 'Markdown' });
});
// Admin help
...
bot.on('message', async (msg) => {
    ...
    const canSend = database_1.db.canUserSendMessage(chatId);
    if (!canSend.allowed) {
        if (canSend.reason === 'free_limit_reached') {
            const message = (0, languages_1.getMessage)(user.language, 'freeLimitReached');
            bot.sendMessage(chatId, message, { parse_mode: 'Markdown' });
        }
            ...
            const expiryDate = user.subscriptionExpiry
                ? user.subscriptionExpiry.toLocaleDateString()
                : 'N/A';
            const message = (0, languages_1.getMessage)(user.language, 'quotaExceeded', { date: expiryDate });
            bot.sendMessage(chatId, message, { parse_mode: 'Markdown' });
        }
        else if (canSend.reason === 'subscription_expired') {
            const message = (0, languages_1.getMessage)(user.language, 'subscriptionExpired');
            bot.sendMessage(chatId, message, { parse_mode: 'Markdown' });
        }
        return;
    }
    ...
    bot.sendMessage(chatId, chooseMessage, { reply_markup: keyboard });
});
...
bot.on('callback_query', async (query) => {
    ...
    if (data.startsWith('lang_')) {
        ...
        database_1.db.updateUser(chatId, { language: langCode });
        const message = (0, languages_1.getMessage)(langCode, 'languageSet', {
            language: languages_1.languages[langCode].name
        });
    ...
    if (data === 'start_config_man' || data === 'start_config_woman') {
        ...
        database_1.db.updateUser(chatId, { userGender });
    ...
    if (data === 'start_target_man' || data === 'start_target_woman') {
        ...
        // Save to database
        database_1.db.updateUser(chatId, { targetGender });
    ...
    if (data === 'keep_response') {
        ...
        database_1.db.addToChatHistory(chatId, 'user', session.messageText);
        database_1.db.addToChatHistory(chatId, 'assistant', session.lastResponse);
        // Increment message count (only charge once they keep it)
        database_1.db.incrementMessageCount(chatId);
    ...
    if (data === 'retry_response') {
        ...
        bot.sendMessage(chatId, '🎯 **PICK YOUR WEAPON**\n\nChoose your response style:', { parse_mode: 'Markdown', reply_markup: keyboard });
        return;
    }
    ...
    if (data.startsWith('level_')) {
        ...
        let user = database_1.db.getUser(chatId);
        if (!user) {
            user = database_1.db.createUser(chatId, query.from.username);
        }
        ...
        const generatingMessage = (0, languages_1.getMessage)(user.language, 'generating');
        bot.editMessageText(generatingMessage, {
            chat_id: chatId,
            message_id: messageId
        });
        try {
            // Generate response using OpenAI
            const languageInstruction = (0, languages_1.getLanguagePrompt)(user.language);
            ...
            const chatHistory = database_1.db.getChatHistory(chatId);
            ...
            const sentMessage = await bot.sendMessage(chatId, `${selectedLevel.name}\n\n"${response}"`, {
                parse_mode: 'Markdown',
                reply_markup: {
                    inline_keyboard: [
                        [
            ...
            const successMessage = (0, languages_1.getMessage)(user.language, 'success');
...
async function generateResponse(messageText, systemPrompt, chatHistory) {
    const messages = [
        {
            role: 'system',
